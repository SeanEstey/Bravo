...
function updateStats(ss_id, archive_ss_id, route) {
  try {
    ...
    if(route_type == 'Res') {
      ...
      Logger.log('core_gifts: %s', String(num_core_gifts));
      Logger.log('route orders: %s', String(route.orders.length));
      Logger.log('num dropoffs: %s', String(num_dropoffs));
...
function updateInventory(ss_id, route) {
  try {
    ...
    Logger.log('Inventory changes: %s ', JSON.stringify(signed_out));
    
    Logger.log('Writing to Row %s', String(dest_row));
